import { NOTES, REMINDERS } from "@/server/endpoints";
...
export default async function handler(req, res) {
  ...
  switch (req.method) {
    case "POST": {
      try {
        const externalRes = await fetch(`${coreUrl}/${REMINDERS.POST}`, {
          method: "POST",
          headers: {
    ...
    case "PUT": {
      try {
        ...
        const externalRes = await fetch(`${coreUrl}/${REMINDERS.PATCH}/${get(req, 'body.id')}`, {
          method: "PUT",
          headers: {
            "Content-Type": "application/json",
            Accept: "application/json",
